/*
This problems come from Topcoder
...
public class Solution{
	...
	private int process(int[] num)
	{
		...
		for(int i = 0; i < size; i++){
			...
			for(int j = 1; j < i; j++){
				if(num[i] > num[j]){
					big[i] = Math.max(small[j]+1,big[i]);					
				}

				if(num[i] < num[j]){
					small[i] = Math.max(big[j] + 1,small[i]);
				}
			}
			ans = Math.max(ans, big[i]);
			ans = Math.max(ans, small[i]);
		}
		return ans;
	}
	...
	public static void main(String[] args)
	{
		...
		int[] num = {1,1,1,1,1};
		System.out.println(s.process(num));
	}
}